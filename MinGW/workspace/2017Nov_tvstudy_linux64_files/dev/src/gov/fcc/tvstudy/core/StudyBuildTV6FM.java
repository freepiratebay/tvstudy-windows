...
public class StudyBuildTV6FM extends StudyBuild {
	...
	public Study buildStudy(StatusLogger status, ErrorLogger errors) {
		...
		if (null != record) {
			defExtDbKey = record.extDb.dbKey;
		} else {
			if ((null != source) && (null != source.extDbKey)) {
				defExtDbKey = source.extDbKey;
			} else {
				if (null != tvExtDb) {
					defExtDbKey = tvExtDb.dbKey;
				}
			}
		}

		Integer theKey =
			Study.createNewStudy(dbID, studyName, Study.STUDY_TYPE_TV6_FM, templateKey, defExtDbKey, true, errors);
		if (null == theKey) {
			return null;
		}
	...
	private boolean buildScenario(ErrorLogger errors) {
		...
		double coChanMX = study.getCoChannelMxDistance();
		double kmPerDeg = study.getKilometersPerDegree();

		// Create the scenario and add that to the study, then add the target.

		ScenarioEditData scenario = new ScenarioEditData(study);
		scenario.name = targetSource.getCallSign() + " and FM IX";
		...
		if (null != tvExtDb) {
			...
			try {
				...
				ExtDbRecordTV.addRecordTypeQueryTV(tvExtDb.type, tvExtDb.version, true, false, q, true);
			} catch (IllegalArgumentException ie) {
			}

			if (ExtDbRecordTV.addRecords(tvExtDb, false, scenario, ExtDbSearch.SEARCH_TYPE_UNDESIREDS, q.toString(),
					null, 0., 0, 0, false, true, true, errors) < 0) {
				return false;
			}
		...
		for (int recIndex = 0; recIndex < recCount; recIndex++) {
			...
			while (fmLit.hasNext()) {
				if (ExtDbRecordFM.areRecordsMX(fmRecord, fmLit.next(), coChanMX, kmPerDeg)) {
					fmLit.remove();
					recCount--;
				}
			}
		...
		if (null != tvExtDb) {
			...
			while (tvLit.hasNext()) {
				...
				for (SourceEditData theSource : tvSources) {
					...
					if (tvRecord.extRecordID.equals(tvSource.extRecordID) ||
							ExtDbRecordTV.areRecordsMX(tvRecord, tvSource, coChanMX, kmPerDeg)) {
						tvLit.remove();
						break;
					}
				}
			...
			for (int recIndex = 0; recIndex < recCount; recIndex++) {
				...
				while (tvLit.hasNext()) {
					if (ExtDbRecordTV.areRecordsMX(tvRecord, tvLit.next(), coChanMX, kmPerDeg)) {
						tvLit.remove();
						recCount--;
					}
				}
