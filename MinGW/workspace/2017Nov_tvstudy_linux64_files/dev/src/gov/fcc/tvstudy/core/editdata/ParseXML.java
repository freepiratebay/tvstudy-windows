	...
	private void doSetup(StudyEditData theStudy, String theDbID, Integer theLookupExtDbKey,
		...
		if (!userRecordMode) {
			...
			if ((null == theLookupExtDbKey) && scenarioMode) {
				theLookupExtDbKey = study.study.extDbKey;
			}
			if (null != theLookupExtDbKey) {
			...
			if (null != theExtDb) {
				...
				if (importMode && importExtDb.dbKey.equals(theExtDb.dbKey)) {
					theExtDb = null;
				}
			}
			...
			if (null != theExtDb) {
				...
				if (scenarioMode) {
					...
					study.loadSharedSourceIndex(lookupIndex.lookupExtDb.dbKey, lookupIndex.sharedSources,
				...
				if (null != theExtDb) {
					...
					if (scenarioMode) {
						...
						study.loadSharedSourceIndex(alternateIndex.lookupExtDb.dbKey, alternateIndex.sharedSources,
		...
		if (qName.equals("SOURCE")) {
			...
			if (userRecordMode || importMode) {
				...
				if ((null != lookupIndex) && (null != extRecordID) &&
					...
					extDbKey = lookupIndex.lookupExtDb.dbKey;
				} else {
					extRecordID = null;
				}
			}
