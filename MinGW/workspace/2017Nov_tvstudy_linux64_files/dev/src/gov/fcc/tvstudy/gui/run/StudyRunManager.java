...
//=====================================================================================================================
// Window to manage and display RunPanel objects managing study build and run processes.  StudyManager instances will
// create and manage a singleton instance of this class to manage all runs from that StudyManager's database.

public class StudyRunManager extends AppFrame {

	public static final String WINDOW_TITLE = "Study Runs";
	...
	public StudyRunManager(AppEditor theParent) {
		...
		listPanel.setBorder(BorderFactory.createTitledBorder("Study runs"));
	...
	private class RunListTableModel extends AbstractTableModel {
		...
		private static final String STUDY_COLUMN = "Study";
		private static final String STATUS_COLUMN = "Status";
		private static final String AUTO_REMOVE_COLUMN = "Auto-remove";
		...
		private JTable createTable() {
			...
			TableColumn theColumn = theTable.getColumn(STUDY_COLUMN);
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Abort the run.  Show a confirmation dialog.  Polling continues while the dialog is up so conditions may change.
	...
	private void doAbort() {
		...
		RunPanel thePanel = currentRunPanel;
		...
		if (0 == JOptionPane.showOptionDialog(this,
				...
				"Abort " + thePanel.getStudyName(), 0, JOptionPane.WARNING_MESSAGE, null, opts, opts[0])) {
			return;
		}
	...
	private void doRemove() {
		...
		parent.editorClosing(currentRunPanel);
	...
	public void addRunPanel(RunPanel thePanel) {

		if (!thePanel.isInitialized()) {
			return;
		}

		int rowIndex = runListModel.add(thePanel);
		thePanel.setParent(this);

		if (rowIndex >= 0) {
			runListTable.setRowSelectionInterval(rowIndex, rowIndex);
			runListTable.scrollRectToVisible(runListTable.getCellRect(rowIndex, 0, true));
		}
	}
	...
	public boolean windowShouldClose() {
		...
		if (!runListModel.isEmpty()) {
			...
			for (RunPanel thePanel : thePanels) {
				if (thePanel.isRunning()) {
					toFront();
					errorReporter.reportMessage("This window can't be closed while a study is running.");
					return false;
				}
			}
