...
// controller does the important window management, calling windowWillOpen() and windowShouldClose() as needed.
...
public abstract class AppFrame extends JFrame implements AppEditor {
	...
	// good practice is to call blockActionsSet() again when closing e.g. windowShouldClose().
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Called by the manager when a window-closing event arrives, if this returns true the window will be hidden, if
	// false the close is aborted.  Subclasses generally override this to check for unsaved changes.
