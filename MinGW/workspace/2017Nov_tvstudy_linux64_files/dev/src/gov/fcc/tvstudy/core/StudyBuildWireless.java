		...
		if (null != record) {
			...
			if ((null != source) && (null != source.extDbKey)) {
				...
				if (null != maskingExtDb) {
					defExtDbKey = maskingExtDb.dbKey;
				}
			}
		}

		Integer theKey =
			Study.createNewStudy(dbID, studyName, Study.STUDY_TYPE_TV_OET74, templateKey, defExtDbKey, true, errors);
		if (null == theKey) {
			return null;
		}
		...
		try {
			...
			if (null != theSource) {
				...
				if ((theSource.channel < minChannel) || (theSource.channel > maxChannel)) {
					...
					scenario.name = theSource.getCallSign() + " wireless IX";
					...
					if (!isAborted() && (wlCount >= 0) && (null != maskingExtDb)) {
						...
						ExtDbRecordTV.addRecordTypeQueryTV(maskingExtDb.type, maskingExtDb.version, true, false, q,
							true);
						tvCount = ExtDbRecordTV.addRecords(maskingExtDb, false, scenario,
							ExtDbSearch.SEARCH_TYPE_UNDESIREDS, q.toString(), null, 0., 0, 0, false, true, true,
							errors);
					}
