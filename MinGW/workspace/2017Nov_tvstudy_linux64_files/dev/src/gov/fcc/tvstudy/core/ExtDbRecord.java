...
public abstract class ExtDbRecord implements Record {
	...
	public static boolean addRecordTypeQuery(int dbType, int version, boolean includePending, boolean includeArchived,
		...
		switch (dbType) {
			...
			case ExtDb.DB_TYPE_LMS:
			case ExtDb.DB_TYPE_LMS_LIVE: {
				return ExtDbRecordTV.addRecordTypeQueryTV(dbType, version, includePending, includeArchived, query,
					combine);
			}
		...
		switch (theExtDb.type) {
			...
			case ExtDb.DB_TYPE_LMS_LIVE: {
				...
				if ((null != theRecord) && (Source.RECORD_TYPE_TV == theRecord.recordType) &&
					...
					query =
					"SELECT " +
						"(CASE WHEN lkp_antenna_field_value.rafv_azimuth = '' THEN 0::FLOAT " +
							"ELSE lkp_antenna_field_value.rafv_azimuth::FLOAT END), " +
						"(CASE WHEN lkp_antenna_field_value.rafv_field_value = '' THEN 0::FLOAT " +
					...
					query =
					"SELECT " +
						"(CASE WHEN aafv_azimuth = '' THEN 0::FLOAT ELSE aafv_azimuth::FLOAT END), " +
						"(CASE WHEN aafv_field_value = '' THEN 0::FLOAT ELSE aafv_field_value::FLOAT END) " +
					"FROM " +
	...
	public String makeCommentText() {
		...
		s.append(AppCore.formatDate(sequenceDate));
	...
	public String getStationData() {
		...
		return ExtDb.getExtDbDescription(extDb.dbID, extDb.dbKey);
	}
