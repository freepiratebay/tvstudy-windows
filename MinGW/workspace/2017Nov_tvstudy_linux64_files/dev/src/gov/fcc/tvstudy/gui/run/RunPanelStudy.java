...
// These are in turn managed by a StudyRunManager window, see details in superclass.  Once set up, this will display a
// ProcessPanel to handle the study engine, and optionally a report area in a separate tab showing report text.  The
// manager will call poll() frequently to keep things moving along.  This implements ProcessController, there is no
// interaction with the engine process but messages are monitored to provide time-to-completion estimates.

public class RunPanelStudy extends RunPanel implements ProcessController, StatusLogger {
	...
	public RunPanelStudy(AppEditor theParent) {
	...
	public boolean initialize(ErrorReporter errors) {
		...
		processPanel = new ProcessPanel(parent, "Study", DbCore.getDbPassword(getDbID()), mergeLineStrings);
	...
	public boolean isDiskSpaceAvailable() {
		...
		return AppCore.isFreeSpaceAvailable(getDbID(), totalSourceCount, 0L);
	}
	...
	public void poll(long now) {
		...
		// Waiting state.  Check to see if the process panel says failed, that means the user pressed the abort button
		// there, proceed to exiting.  Otherwise check with AppTask to see if this can start, if not, wait.

		if (RUN_STATE_WAIT == runState) {

			if (processPanel.didProcessFail()) {
		...
		// build state just return until the thread exits.  Check for abort while the thread is running, if seen set
		// the abort flag in the build object, if it is able to abort that will be picked up on a later poll.
		...
		if (RUN_STATE_BUILD == runState) {
			...
			if (null == buildThread) {
				...
				if (buildThread.isAlive()) {
					if (!studyBuild.isAborted() && processPanel.didProcessFail()) {
						studyBuild.abort();
					}
					return;
				}
				...
				// manager about the new study lock state, then proceed to start.  The AppPanel original parent set at
				// construction is the study manager.  If this was just a build process, cancel the pending run state
				// in the process panel and proceed to exit.
				...
				if (buildFailed) {
					...
					if (runAfterBuild) {
						...
						runState = RUN_STATE_EXITING;
						runFailed = !processPanel.cancelProcess();
					}
				}
		...
		// process will start when the panel is polled below.  Again check for an abort from the process panel.
		...
		// This has to be done directly here rather than letting the study manager do it when editorClosing() is
		// called, because the lock may be in one of two possible states.  If the initial lock was exclusive, the
		// engine downgrades to shared and increments the lock count.  However this can't assume the lock will be in
		// that state, because the engine may have aborted before it got around to downgrading the lock.  First look
		// for an exact match to the lock as originally set, if that does not match and the original lock was
		// exclusive, then check for a shared lock with a lock count one higher.  Note errors do not cause the failed
		// state to be set; the run is already done and it's success or failure state should not be altered.
		...
		// Notify the task queue this task is done, and the study manager that the lock is released.
	...
	private void buildStudy() {
		...
		if (null == theStudy) {
			...
			DbConnection db = DbCore.connectDb(getDbID(), buildErrors);
			if (null != db) {
				try {
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Abort the run if possible.  This is a request and is not guaranteed to be effective.  Polling must always
	// continue until isRunning() returns false.  If the abort works that will happen quickly and the final state will
	// be failed.  This does nothing in init, exiting, or exit.  In wait, build before the build thread has been
	// started, or start, just set exiting, the next poll will shut things down.  In build after the build thread has
	// started set the StudyBuild object's abort flag, it will abort and return failure if/when possible, a future poll
	// will handle that.  If in running send a kill to the process, again a future poll will handle that as a failure.

	public void abort() {

		if ((RUN_STATE_WAIT == runState) || ((RUN_STATE_BUILD == runState) && (null == buildThread)) ||
				(RUN_STATE_START == runState)) {
			runState = RUN_STATE_EXITING;
			runFailed = true;
			processPanel.setStatusMessage("Study run aborted");
			return;
		}
		if (RUN_STATE_BUILD == runState) {
			studyBuild.abort();
