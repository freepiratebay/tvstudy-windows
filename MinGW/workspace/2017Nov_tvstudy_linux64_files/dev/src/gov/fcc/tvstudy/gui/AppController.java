...
public class AppController implements WindowListener {
	...
	private static class PrefsDialog extends AppDialog {
		...
		private PrefsDialog() {
			...
			ixCheckIncludeForeignDefaultCheckBox =
				new JCheckBox("IX check study: Include non-U.S. stations by default");
			ixCheckIncludeForeignDefaultCheckBox.setFocusable(false);

			ixCheckDefaultCPExcludesBLCheckBox =
				new JCheckBox("IX check study: CP records exclude baseline by default");
	...
	public static void checkInstall() {
		...
		arguments.add(AppCore.libDirectoryPath + File.separator + "check_install.exe");
		arguments.add(AppCore.workingDirectoryPath);
		AppController.showWindow(new ToolRun("Installation Check", arguments));
	}
	...
	//-----------------------------------------------------------------------------------------------------------------
	// When a AppFrame or AppDialog is closing, give it a chance to clean up or cancel the close.
