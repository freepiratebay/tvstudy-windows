...
public class ExtDbRecordTV extends ExtDbRecord implements Record {
	...
	private static final String[] LMS_APP_STATUS_CODES = {
		"SAV", "SUB", "DIS", "WIT", "DEN", "RET", "CAN", "REC", "REV"
	};
	...
	// List of facility IDs of stations that can be sharing guests in LMS.  If application.channel_sharing_ind = 'Y',
	// then if the facility ID is in this list the record is assumed to be a guest and ignored, else it is a host.
	...
	private static LinkedList<ExtDbRecordTV> findRecordsTV(ExtDb extDb, String query, GeoPoint searchCenter,
		...
		// Load sharing guest facility ID list on first call.

		if (null == sharingGuestFacilityIDs) {
			sharingGuestFacilityIDs = new HashSet<Integer>();
			try {
				BufferedReader reader = new BufferedReader(new FileReader(new File(AppCore.libDirectoryPath +
					File.separator + SHARING_GUEST_FILE_NAME)));
				String id;
				while (true) {
					id = AppCore.readLineSkipComments(reader);
					if (null == id) {
						break;
					}
					try {
						sharingGuestFacilityIDs.add(Integer.valueOf(id));
		...
		if (isCDBS) {
			...
			if (ExtDb.DB_TYPE_LMS_LIVE == extDb.type) {
				...
				"SELECT " +
					...
					"(CASE WHEN app_location.aloc_lat_deg IN ('','null') THEN 0::INT " +
						"ELSE app_location.aloc_lat_deg::INT END), " +
					"(CASE WHEN app_location.aloc_lat_mm IN ('','null') THEN 0::INT " +
						"ELSE app_location.aloc_lat_mm::INT END), " +
					"(CASE WHEN app_location.aloc_lat_ss IN ('','null') THEN 0::FLOAT " +
						"ELSE app_location.aloc_lat_ss::FLOAT END), " +
					"app_location.aloc_long_dir, " +
					"(CASE WHEN app_location.aloc_long_deg IN ('','null') THEN 0::INT " +
						"ELSE app_location.aloc_long_deg::INT END), " +
					"(CASE WHEN app_location.aloc_long_mm IN ('','null') THEN 0::INT " +
						"ELSE app_location.aloc_long_mm::INT END), " +
					"(CASE WHEN app_location.aloc_long_ss IN ('','null') THEN 0::FLOAT " +
						"ELSE app_location.aloc_long_ss::FLOAT END), " +
					"(CASE WHEN app_antenna.aant_rc_amsl IN ('','null') THEN 0::FLOAT " +
						"ELSE app_antenna.aant_rc_amsl::FLOAT END), " +
					"(CASE WHEN app_antenna.aant_rc_haat IN ('','null') THEN 0::FLOAT " +
						"ELSE app_antenna.aant_rc_haat::FLOAT END), " +
					"(CASE WHEN app_antenna_frequency.aafq_power_erp_kw IN ('','null') THEN 0::FLOAT " +
						"ELSE app_antenna_frequency.aafq_power_erp_kw::FLOAT END), " +
					"0::FLOAT AS max_erp_any_angle, " +
					"app_antenna.aant_antenna_record_id, " +
					"(CASE WHEN app_antenna.aant_rotation_deg IN ('','null') THEN 0::FLOAT " +
						"ELSE app_antenna.aant_rotation_deg::FLOAT END), " +
					"app_antenna.aant_antenna_record_id, " +
					"(CASE WHEN app_antenna.aant_electrical_deg IN ('','null') THEN 0::FLOAT " +
						"ELSE app_antenna.aant_electrical_deg::FLOAT END), " +
					"(CASE WHEN app_antenna.aant_mechanical_deg IN ('','null') THEN 0::FLOAT " +
						"ELSE app_antenna.aant_mechanical_deg::FLOAT END), " +
					"(CASE WHEN app_antenna.aant_true_deg IN ('','null') THEN 0::FLOAT " +
					...
					"(CASE WHEN application_facility.afac_channel IN ('','null') THEN 0::INT " +
					...
					"(CASE WHEN app_antenna.foreign_station_beam_tilt IN ('','null') THEN 0::FLOAT " +
		...
		// query argument, if any.  This allows the argument to be null/empty to match all records.
			...
			if (isCDBS) {
				...
				if (extDb.version < 1) {
					...
					fldStr = fldStr + ", " +
						"application.channel_sharing_ind, " +
		...
		if (null != db) {
			try {
				...
				double dtsDist;
				boolean drtFlag, isArch, isPend, isHost;
				...
				while (db.next()) {
					...
					if (null != dtsParent) {
						...
						// Ignore sharing guest records, set flag on sharing host records.

						isHost = false;
						str = db.getString(43);
						if ((null != str) && str.equalsIgnoreCase("Y")) {
							if (sharingGuestFacilityIDs.contains(Integer.valueOf(facID))) {
								continue;
							}
							isHost = true;
						}
						...
						// status is APP.  If "PEN" appears in current_status_code the status is APP if auth_type_code
						// is "C".  Otherwise "C" in the auth_type_code field means CP, "L" means LIC.  Note "PEN" in
						// current_status_code also causes isPending to be set below, independent of the result here.
					...
					theRecord.isSharingHost = isHost;
	...
	private static LinkedList<ExtDbRecordTV> findBaselineRecords(ExtDb extDb, String query, GeoPoint searchCenter,
		...
		if (isCDBS) {
			...
			if (ExtDb.DB_TYPE_LMS_LIVE == extDb.type) {
				...
				fldStr =
				"SELECT " +
					"(CASE WHEN app_dtv_channel_assignment.adca_facility_record_id IN ('','null') THEN 0::INT " +
				...
				" FROM " +
					...
					"JOIN common_schema.facility ON " +
						"(CASE WHEN app_dtv_channel_assignment.adca_facility_record_id IN ('','null') THEN false " +
	...
	public static boolean addChannelQueryTV(int dbType, int version, int channel, int minimumChannel,
		...
		if ((ExtDb.DB_TYPE_CDBS == dbType) || (ExtDb.DB_TYPE_LMS == dbType) || (ExtDb.DB_TYPE_LMS_LIVE == dbType)) {
			...
			if (ExtDb.DB_TYPE_CDBS == dbType) {
				...
				if (ExtDb.DB_TYPE_LMS_LIVE == dbType) {
					query.append("(CASE WHEN application_facility.afac_channel IN ('','null') THEN false " +
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Add record type search query clause.  Archived records may optionally be included.  Any unknown record types
	// are treated as current, so this excludes types not desired rather than including those desired.  Note this has
	// degenerated into a no-op unless includeArchived is true, but many query builds still always call this.

	public static boolean addRecordTypeQueryTV(int dbType, int version, boolean includeArchived, StringBuilder query,
			boolean combine) {
	...
	public static boolean addBaselineFacilityIDQuery(int dbType, int version, int facilityID, StringBuilder query,
		...
		if ((ExtDb.DB_TYPE_CDBS == dbType) || (ExtDb.DB_TYPE_LMS == dbType) || (ExtDb.DB_TYPE_LMS_LIVE == dbType)) {
			...
			if (ExtDb.DB_TYPE_CDBS == dbType) {
				...
				if (ExtDb.DB_TYPE_LMS_LIVE == dbType) {
					query.append("(CASE WHEN app_dtv_channel_assignment.adca_facility_record_id IN ('','null') " +
						"THEN false ELSE (app_dtv_channel_assignment.adca_facility_record_id::INT = ");
					query.append(String.valueOf(facilityID));
					query.append(") END)");
				} else {
	...
	public static int addRecords(ExtDb extDb, boolean useBaseline, ScenarioEditData scenario, int searchType,
			...
			boolean disableMX, boolean mxFacilityIDOnly, boolean preferOperating, boolean setUndesired,
			ErrorLogger errors) {
		...
		if (Study.STUDY_TYPE_TV_IX == studyType) {
			...
			while (lit.hasNext()) {
				theRecord = lit.next();
				if (((null != protectedSource) && areRecordsMX(theRecord, protectedSource, true, 0., 0.)) ||
						((null != proposalSource) && areRecordsMX(theRecord, proposalSource, true, 0., 0.))) {
					lit.remove();
				}
			}
		}
	...
	public boolean updateSource(SourceEditDataTV theSource, ErrorLogger errors) {
		...
		// is still current, also look for DTV records with license status.  In either case only active records are
		// considered; unlike CDBS, flagged LMS reference facilities are still active, but are treated as archived by
		// other queries, see addRecordTypeQuery().  If multiple matches use the most recent.
			...
			if (isCDBS) {
				...
				theRecs = findRecordsTV(extDb, "(application_facility.afac_facility_id = " + facilityID + ") AND " +
					...
					"((application.dts_reference_ind = 'R') OR " +
						"(license_filing_version.auth_type_code = 'L'))", null, 0., 0., null, errors);
				if (null == theRecs) {
					return false;
				}
			}
	...
	// check here when a DTS is involved, only the DTS reference point coordinates are ever checked.  Optionally the
	// backup matching city-state and distance checks may be skipped so only facility ID is tested, also the distance
	// check value is set by parameter and can be disabled by setting to 0.  DRT records get separate handling; a
	// parent full-service and it's DRTs have the same facility ID but those are not necessarily MX.  A DRT is never MX
	// to it's parent under any conditions.  Two DRTs for the same parent are MX if co-channel and with matching site
	// number, or less than a fixed distance apart.  That distance test will always be applied.

	public static boolean areRecordsMX(ExtDbRecordTV a, ExtDbRecordTV b, boolean facIDOnly, double mxDist,
			double kmPerDeg) {
		...
		return areRecordsMX(
			a.facilityID, a.isDRT, a.siteNumber, a_channel, a.country.key, a.state, a.city, a.location,
			b.facilityID, b.isDRT, b.siteNumber, b_channel, b.country.key, b.state, b.city, b.location,
			facIDOnly, mxDist, kmPerDeg);
	}

	public static boolean areRecordsMX(ExtDbRecordTV a, SourceEditDataTV b, boolean facIDOnly, double mxDist,
			double kmPerDeg) {
		int a_channel = (a.replicateToChannel > 0) ? a.replicateToChannel : a.channel;
		return areRecordsMX(
			a.facilityID, a.isDRT, a.siteNumber, a_channel, a.country.key, a.state, a.city, a.location,
			b.facilityID, b.isDRT, b.siteNumber, b.channel, b.country.key, b.state, b.city, b.location,
			facIDOnly, mxDist, kmPerDeg);
	}

	public static boolean areRecordsMX(SourceEditDataTV a, SourceEditDataTV b, boolean facIDOnly, double mxDist,
			double kmPerDeg) {
		return areRecordsMX(
			a.facilityID, a.isDRT, a.siteNumber, a.channel, a.country.key, a.state, a.city, a.location,
			b.facilityID, b.isDRT, b.siteNumber, b.channel, b.country.key, b.state, b.city, b.location,
			facIDOnly, mxDist, kmPerDeg);
	}

	private static boolean areRecordsMX(int a_facilityID, boolean a_is_drt, int a_site_number, int a_channel,
			int a_country_key, String a_state, String a_city, GeoPoint a_location, int b_facilityID, boolean b_is_drt,
			int b_site_number, int b_channel, int b_country_key, String b_state, String b_city, GeoPoint b_location,
			boolean facIDOnly, double mxDist, double kmPerDeg) {
		...
		if (a_facilityID == b_facilityID) {
			...
			if (a_is_drt || b_is_drt) {
				...
				if (a_site_number == b_site_number) {
					return true;
				}

				if (a_location.distanceTo(b_location, kmPerDeg) < DRT_MX_DISTANCE) {
					return true;
				}
