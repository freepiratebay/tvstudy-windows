...
public class StudyEditData {
	...
	// Mutable properties, study name, description, default data set, mode, points set, area mode and geography, and
	// output settings.
	...
	public synchronized SourceEditData findSharedSource(Integer theExtDbKey, String theExtRecordID) {

		if ((null == theExtDbKey) || (null == theExtRecordID)) {
			return null;
		}

		SourceIndex theIndex = extDbsIndex.get(theExtDbKey);
		if (null == theIndex) {
			return null;
		}

		return theIndex.getSource(theExtRecordID);
	}
	...
	public synchronized SourceEditDataTV findSharedReplicationSource(Integer theExtDbKey, String theExtRecordID,
			int channel) {
		...
		return theIndex.getReplicationSource(theExtRecordID, channel);
	}
	...
	public boolean isDataChanged() {
		...
		if (null == study.extDbKey) {
			...
			if (null == extDbKey) {
				...
				if (!extDbKey.equals(study.extDbKey)) {
					return true;
				}
			}
		}
		...
		if (null != db) {
			try {
				...
				if (!delKeys.isEmpty()) {
					...
					db.update("DELETE FROM ix_rule WHERE ix_rule_key IN " + DbConnection.makeKeyList(delKeys));
				...
				if (!delKeys.isEmpty()) {
					...
					keyList = DbConnection.makeKeyList(delKeys);
				...
				if (!deletedSourceKeys.isEmpty()) {
					...
					keyList = DbConnection.makeKeyList(deletedSourceKeys);
				...
				"UPDATE study SET " +
					...
					"mod_count = " + modCount + "," +
					"ext_db_key = " + ((null == extDbKey) ? "0" : String.valueOf(extDbKey)) + "," +
				...
				study = new Study(dbID, study.key, name, description, study.studyType, studyMode, study.templateKey,
					study.templateName, study.templateLocked, extDbKey, thePointSetKey, propagationModel,
	...
	public Integer readScenariosFromXML(Integer theExtDbKey, Integer altExtDbKey, Reader xml) {
		return readScenariosFromXML(theExtDbKey, altExtDbKey, xml, null);
	}

	public Integer readScenariosFromXML(Integer theExtDbKey, Integer altExtDbKey, Reader xml, ErrorLogger errors) {
		...
		try {
			...
			ParseXML handler = new ParseXML(this, theExtDbKey, altExtDbKey, errors);
