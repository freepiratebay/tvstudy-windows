	...
	private void doOpRun() {
		...
		try {
			...
			if (!isSrc) {
				...
				ExtDbRecord.addRecordTypeQuery(dbType, version, includeArchived, query, true);
			}
		...
		if (1 == recordCount) {
			if (null == extDb) {
				...
				parameters.put(KEY_EXT_DB_KEY, String.valueOf(extDb.key));
		...
		if (null == extDb) {
			...
			page.append("<input type=\"hidden\" name=\"" + KEY_EXT_DB_KEY + "\" value=\"" + extDb.key + "\">\n");
		}
			...
			if (null != value) {
				try {
					if (extDb.isGeneric()) {
						...
						ExtDbRecord.addRecordTypeQuery(extDb.type, extDb.version, false, query, true);
					}
				} catch (IllegalArgumentException ie) {
					theResult.errorMessage = "ERROR: " + ie.getMessage();
					return theResult;
