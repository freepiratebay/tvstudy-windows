...
public class DbController {
	...
	private static class LoginDialog extends AppDialog {
		...
		private void doOpenDb(boolean doManage) {
			...
			// Check for an existing database manager, bring to front.  Note there are unusual cases in which an
			// existing manager dialog may be identified by either the hostname or UUID, so try both. UUID is valid.
			...
			// Check if the database is already registered, if so and management was requested, show message indicating
			// management can't be performed until the database is no longer in use by any object.  Otherwise create or
			// bring to front a study manager window.

			if (DbCore.isDbRegistered(theInfo.dbID)) {
				if (doManage) {
					errorReporter.reportWarning("That database is currently in use, please try again later.");
					return;
				}
			...
			if (doManage || theInfo.needsManage) {
				...
				if (DbCore.registerDb(theInfo, errorReporter)) {
					if (StudyManager.showManager(theInfo.dbID)) {
						AppController.hideWindow(this);
					}
					return;
	...
	private static class ManagerDialog extends AppDialog {
		...
		private void doOpen() {
			...
			if (!DbCore.registerDb(dbInfo, errorReporter)) {
				return;
			}
