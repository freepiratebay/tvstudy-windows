...
public class RecordFindPanel extends AppPanel implements ExtDbListener {
	...
	private void doSearch() {
		...
		if (!useBaseline || (Source.RECORD_TYPE_TV != searchRecordType)) {
			...
			try {
				...
				if (!isSrc) {
					if (ExtDbRecord.addRecordTypeQuery(dbType, version, includeArchivedCheckBox.isSelected(), query,
							hasCrit)) {
						hasCrit = true;
					}
				}
	...
	private void doImport() {
		...
		PickExtDbDialog promptDialog = new PickExtDbDialog(this, title, defaultExtDbKey) {
	...
	private void doExtDbUpdate(boolean firstUpdate) {
		...
		// The user records item for each type is added before the first actual data set in the list, which puts it
		// just after data sets with keys in the reserved range which represent special functions like most-recent or
		// live servers; getExtDbList() always puts those at the top.

		if (((0 == recordType) || (Source.RECORD_TYPE_TV == recordType)) &&
				((0 == studyType) || Study.isRecordTypeAllowed(studyType, Source.RECORD_TYPE_TV))) {
			boolean doUser = true;
			if (!userRecordsOnly) {
				addList = ExtDb.getExtDbList(getDbID(), Source.RECORD_TYPE_TV, errors);
				if (null != addList) {
		...
		if (((0 == recordType) || (Source.RECORD_TYPE_WL == recordType)) &&
			...
			boolean doUser = true;
			if (!userRecordsOnly) {
				addList = ExtDb.getExtDbList(getDbID(), Source.RECORD_TYPE_WL, errors);
				if (null != addList) {
					for (KeyedRecord theDb : addList) {
						if (doUser && ((theDb.key < ExtDb.RESERVED_KEY_RANGE_START) ||
								(theDb.key > ExtDb.RESERVED_KEY_RANGE_END))) {
		...
		if (((0 == recordType) || (Source.RECORD_TYPE_FM == recordType)) &&
			...
			boolean doUser = true;
			if (!userRecordsOnly) {
				addList = ExtDb.getExtDbList(getDbID(), Source.RECORD_TYPE_FM, errors);
				if (null != addList) {
					for (KeyedRecord theDb : addList) {
						if (doUser && ((theDb.key < ExtDb.RESERVED_KEY_RANGE_START) ||
								(theDb.key > ExtDb.RESERVED_KEY_RANGE_END))) {
