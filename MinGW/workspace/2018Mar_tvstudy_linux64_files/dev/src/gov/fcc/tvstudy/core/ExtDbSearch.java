...
public class ExtDbSearch {
	...
	public ExtDbSearch copy() {
		...
		newSearch.mxFacilityIDOnly = mxFacilityIDOnly;
	...
	public synchronized boolean save(String theDbID, ErrorLogger errors) {
		...
		if (name.length() > DbCore.NAME_MAX_LENGTH) {
			name = name.substring(DbCore.NAME_MAX_LENGTH);
		}
		...
		if (null != db) {
			try {
				...
				while (db.next()) {
					...
					search.mxFacilityIDOnly = db.getBoolean(5);
					search.preferOperating = db.getBoolean(6);
					search.desiredOnly = db.getBoolean(7);
					...
					for (String s : db.getString(10).split(",")) {
						try {
					...
					search.radius = db.getDouble(11);
					search.center.setLatLon(db.getDouble(12), db.getDouble(13));

					search.minimumChannel = db.getInt(14);
					search.maximumChannel = db.getInt(15);

					search.additionalSQL = db.getString(16);

					search.autoRun = db.getBoolean(17);
	...
	public String makeQuery(ExtDb extDb, boolean useBaseline, ErrorLogger errors) {
		...
		try {
			...
			if (!useBL && !isGen) {
				hasCrit = ExtDbRecord.addRecordTypeQuery(extDb.type, extDb.version, false, q, false);
			}
