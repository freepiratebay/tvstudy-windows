...
public class TemplateManager extends AppFrame {
	...
	private void doRenameTemplate() {
		...
		while (true) {
			...
			if (Template.checkTemplateName(getDbID(), theName, theItem.name, errorReporter)) {
				break;
			}
		};

		Template.renameTemplate(getDbID(), theItem.key.intValue(), theName, errorReporter);
	...
	private void doDuplicateTemplate() {
		...
		while (true) {
			...
			theName = theName.trim();
			if (Template.checkTemplateName(getDbID(), theName, errorReporter)) {
				break;
			}
		};
