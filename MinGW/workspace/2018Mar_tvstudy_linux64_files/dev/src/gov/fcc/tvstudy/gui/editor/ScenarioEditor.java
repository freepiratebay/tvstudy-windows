...
public class ScenarioEditor extends AppFrame {
	...
	private void doAddSources(ExtDbSearch search) {
		...
		if ((null != search) && search.autoRun && (null != scenario.study.extDbKey)) {
			ExtDb theDb = ExtDb.getExtDb(getDbID(), scenario.study.extDbKey, errorReporter);
			if (null != theDb) {
	...
	private void doToggleFlags(boolean doDes) {
		...
		for (int row : sourceTable.getSelectedRows()) {
			...
			theItem = sourceModel.get(rowIndex);
			if (doDes) {
				sourceModel.setIsDesired(rowIndex, !theItem.isDesired);
			} else {
				sourceModel.setIsUndesired(rowIndex, !theItem.isUndesired);
			}
		}
	}
