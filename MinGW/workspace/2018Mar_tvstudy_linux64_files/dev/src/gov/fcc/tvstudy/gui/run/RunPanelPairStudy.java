...
public class RunPanelPairStudy extends RunPanel implements StudyLockHolder, ProcessController {
	...
	public boolean initialize(ErrorReporter errors) {
		...
		filePath = AppCore.outDirectoryPath + File.separator + DbCore.getHostDbName(dbID) + File.separator + studyName;
		...
		baselinePanel = new ProcessPanel(this, "Study", DbCore.getDbPassword(dbID), null);
	...
	private void startRunning() {
		...
		if (!error) {
			...
			for (int runNumber = 0; runNumber < runCount; runNumber++) {
				...
				theRun = new ProcessPanel(this, "Study", DbCore.getDbPassword(dbID), null);
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Abort action from a process panel; in any but the build state this just loops back to the panel.  During build
	// forward the abort to the build object.
	...
	// thread has been created, just set exiting.  In build if the thread is running, send the abort to the build
	// object, including a thread interrupt if the build object indicates it is not in a cancelable state.  In prerun,
	// running, or postrun, send a stop to all processes.  Prompt the user for confirmation if processes will be
	// stopped or the build object indicates it is not cancelable.

	public void abort() {

		if ((RUN_STATE_INIT == runState) || (RUN_STATE_RESTORE == runState) || (RUN_STATE_EXITING == runState) ||
				(RUN_STATE_EXIT == runState)) {
			return;
		}
		...
		if (RUN_STATE_BUILD == runState) {
			willCancel = studyBuild.abortWillCancel();
		}
		...
		if (!willCancel) {
			...
			if (0 == JOptionPane.showOptionDialog(this,
					"This will forcibly terminate all running studies.  Output\n" +
