...
// stopProcess().  Status is returned by isProcessRunning(), that will return true even before the process starts.
// Once isProcessRunning() returns false, didProcessFail() indicates if the process failed to start, was killed, or
// exited with a non-zero result; wasProcessCanceled() indicates if it was stopped before starting.  After the process
// ends, hasOutput() will indicate if there is output accumulated regardless of success or failure.  The output can be
// saved with writeOutputTo(), or the saveOutput() method will prompt the user to choose a file and write the output.
...
// the panel is displayable and otherwise functional but it will not start a process.  The method displayLogMessage()
...
public class ProcessPanel extends AppPanel {
	...
	public ProcessPanel(AppEditor theParent, String theProcessName, String thePassword,
		...
		// Abort button.  Until the process starts this is a cancel button, see stopProcess().  If a process controller
		// is set that handles the action, it must call stopProcess() here if needed.
		...
		abortButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent theEvent) {
				if (null != controller) {
					controller.stopProcess(outerThis);
				} else {
					stopProcess(true);
				}
			}
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Change label of the abort button.

	public void setAbortButtonLabel(String theLabel) {

		abortButton.setText(theLabel);
	}
	...
	public boolean pollProcess(long now) throws IOException {
		...
		if (!processStarted) {
			...
			if (processRunning) {
				...
				statusLabel.setText(processName + " running");
				...
				statusLabel.setText(processName + " startup failed");
	...
	//-----------------------------------------------------------------------------------------------------------------
	// Stop the process, or cancel startup if that has not yet happened.  Return true if it is canceled, killed, or
	// has already exited.  If the process is running this can show a confirmation prompt, if the user says No (the
	// default) this returns false.

	public boolean stopProcess(boolean confirm) {
		...
		if (!processStarted) {
			...
			statusLabel.setText(processName + " canceled");
			abortButton.setEnabled(false);
			...
			// Presumably polling continued while the dialog was up so re-check processRunning.
			...
			if (processRunning) {
				...
				statusLabel.setText(processName + " aborted");
				abortButton.setEnabled(false);
	...
	public boolean wasProcessCanceled() {
